    ...
    <div class="row">
      <div class="col-12">
        ...
        <header class="banner d-flex justify-content-end align-items-center">
          <a class="brand me-auto" href="{{ home_url('/') }}">
            @if($header_logo)
          ...
          <div class="nav-menu-wrap me-3 d-none d-md-block">
            @if($call_or_text_title && $header_phone_number)
              {!! $call_or_text_title !!} <span class="font-weight-700 blue">{!! $header_phone_number !!}</span>
            @endif
          </div>

          @if($pay_your_bill_title)
            <a href="{!! $bill_payment_link !!}" class="button is-icon is-white-shadowed is-small w-inline-block" target="_blank">
          @else
      ...
      <div class="col-12 d-md-none py-2 mt-1 text-size-tiny-14 text-center white" style="background-color: {!! $secondary_color !!}">
        @if($call_or_text_title && $header_phone_number)
          {!! $call_or_text_title !!} <span class="font-weight-700">{!! $header_phone_number !!}</span>
        @endif
      </div>
    </div>
  </div>
