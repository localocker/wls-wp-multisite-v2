@if(is_page('full-form'))
  ...
  <?php
    ...
    $banner_image_full_url = wp_get_attachment_image_src(get_field('banner_image', 'option'), 'full');
    $banner_image_large_url = wp_get_attachment_image_src(get_field('banner_image', 'option'), 'large');
    $banner_image_medium_url = wp_get_attachment_image_src(get_field('banner_image', 'option'), 'medium');
  ...
  <div class="container-fluid py-5 page-header-bg-color">
    <div class="row">
      ...
      <div class="col-12 text-center">
        @if(is_archive())
          <h1 class="m-0 white">{!! post_type_archive_title( '', false ) !!}</h1>
        @else
          <h1 class="m-0 white">{!! $title !!}</h1>
        @endif

        @if(is_home() || is_front_page())
          <h2 class="h4 font-weight-500">Reserve Your Unit</h2>
        @elseif(is_search() || is_post_type_archive('storage-unit'))
          <h2 class="h4 font-weight-500">Search Results</h2>
        @elseif(is_page('privacy-policy') || is_page('terms'))
