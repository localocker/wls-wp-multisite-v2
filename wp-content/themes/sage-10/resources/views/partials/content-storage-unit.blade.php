...
<div class="filtered-item @unless($available_for_move_in) occupied @endunless">
  ...
  <div class="filter-item-image-container position-relative">
    <div href="#" class="button is-icon is-smaller-light w-inline-block size-link">
      ...
      <div>
        @if($dimensions && in_array('small', $dimensions))
          Small
        @elseif($dimensions && in_array('medium', $dimensions))
          Medium
        @elseif($dimensions && in_array('large', $dimensions))
          Large
        @elseif($dimensions && in_array('boat', $dimensions))
          Boat
        @elseif($dimensions && in_array('rv', $dimensions))
          RV
        @endif
      </div>
    </div>
    ...
    @if (has_post_thumbnail())
      <a href="{!! $unit_featured_img !!}" title="View Larger Image" class="venobox">
  ...
  <div class="filtered-item-details-container">
    <div class="filtered-item-details-wrap">
      <div class="filter-item-detail size">
        ...
        <div>
          <h5 class="text-size-medium-24 text-weight-bold main-color">
            @if($size)
              {!! $size !!} <span class="text-weight-normal">- (Unit {!! the_title() !!})</span>
            @endif
          </h5>
          <div class="text-size-small-16 text-weight-normal text-color-grey line-height-1-2">
            @if($unit_type)
              {!! $unit_type !!}
            @endif
          </div>
        ...
        <button href="#" class="button is-icon is-smaller size-help w-inline-block" data-bs-toggle="modal" data-bs-target="#sizeModal">
          <div class="small-white-icon-box">
            <div class="icon-1x1-small blk w-embed">
    ...
    <div class="filtered-item-details-wrap size-help-individual">
      <div class="filter-item-detail size-help">
        <button class="button is-icon is-smaller hidden w-inline-block" data-bs-toggle="modal" data-bs-target="#sizeModal">
          <div class="small-white-icon-box">
            <div class="icon-1x1-small blk w-embed">
              <svg width="17px" height="17px" stroke-width="1.5" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="#000000">
                <path d="M9 9L4 4M4 4V8M4 4H8" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
    ...
    <div class="filtered-item-details-wrap">
      <div class="filter-item-detail">
        @if($promo_discount_or_header)
          <div class="text-size-regular-20 text-color-red text-weight-semibold line-height-1-2 montserrat">
            {!! $promo_discount_or_header !!}
          </div>
        @endif

        @if($promotional_text)
          <div class="text-size-small-16 text-color-red line-height-1-2">
            {!! $promotional_text !!}
          </div>
        @endif
      </div>
    ...
    <div class="filtered-item-details-wrap">
      <div class="filter-item-detail right-flex-end-justified">
        ...
        <div class="reserve-now">
          @if($available_for_move_in)
            <a href="/checkout/full-form/?unit_id={!! $unit_id !!}" class="button is-blue is-icon is-smaller mt-1 w-inline-block">
          @else
    ...
    <div class="filtered-item-details-wrap reserve-now-individual">
      <div class="filter-item-detail">
        @if($available_for_move_in)
          <a href="/checkout/full-form/?unit_id={!! $unit_id !!}" class="button is-blue is-smaller w-inline-block">
        @else
