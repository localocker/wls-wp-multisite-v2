...
class WpGetApi_Extras_Wpdatatables {
		...
		if ( is_plugin_active( 'wpdatatables/wpdatatables.php' ) ) {
			...
			if ( isset( $_GET['page'] ) && strpos( $_GET['page'], 'wpgetapi_' ) !== false ) {
				...
				$fields = array(
					...
					array(
						'name'       => 'Root' . '<span class="dashicons dashicons-editor-help" data-tip="' . __( 'If there are issues creating the table, try setting the default root. Format should be like <b>root->location</b> where location is the name of the first item in the JSON. If left blank, <b>root</b> is the default.', 'wpgetapi' ) . '"></span>',
		...
		?>
			...
			<div class="cmb-th">
				<label>
					...
					<span class="dashicons dashicons-editor-help" data-tip="<?php _e( 'Click the button to create a new wpDataTable from the endpoint data. Ensure that you have tested the endpoint and it is returning the data you want.', 'wpgetapi' ); ?>"></span>
				</label>
			</div>
	...
	public function get_field_options() {
		...
		if ( $all_tables ) {
			foreach ( $all_tables as $table ) {
				$tables[ $table['id'] ] = esc_html__( 'wpDataTables - ', 'wpgetapi-extras' ) . $table['title'] . ' (ID ' . $table['id'] . ')';
			}
		} else {
	...
	public function create_initial_file() {
		...
		// update the content
		$content = array(
			'url'           => $this->upload_url() . '/wpdatatables-' . $table_id . '.json',
			'method'        => 'get',
			'authOption'    => '',
			'username'      => '',
			'password'      => '',
