...
<div>
	...
	<p>
		<?php
			esc_html_e( 'PHP is the scripting language that powers WordPress under the hood. New versions are released over time that brings both speed and security improvements. It’s important to use the latest and greatest tools, so older PHP versions eventually cease to be supported. Shipper is using AWS PHP SDK which requires PHP 7.4 or higher to work properly. ', 'shipper' );
		?>
	</p>
				...
				<p>
					...
					echo wp_kses_post(
						sprintf(
							...
							__( '<b>%1$s</b> is using PHP %2$s, and Shipper needs PHP 7.4 or above to work.', 'shipper' ),
							$domain,
							$value
						)
					);
